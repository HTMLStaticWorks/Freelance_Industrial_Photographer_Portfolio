/**
 * INDUS PHOTO - INDUSTRIAL PHOTOGRAPHY PORTFOLIO
...
function initMobileMenu() {
  ...
  // Move RTL and Theme toggles into mobile menu on <=991px screens
  const moveToggles = () => {
    ...
    if (window.innerWidth <= 991) {
      if (rtlToggle && rtlToggle.parentElement === headerActions) {
        ...
        if (!toggleWrapper) {
          ...
          toggleWrapper.style.marginTop = '1.25rem';
          toggleWrapper.style.paddingTop = '1.25rem';
          toggleWrapper.style.borderTop = '1px solid var(--border-color)';
        }
        
        const loginBtn = navMenu.querySelector('.login-mobile-btn');
        if (loginBtn) {
          navMenu.insertBefore(toggleWrapper, loginBtn);
        } else {
...
function initPortfolioFilterAndLightbox() {
  ...
  const lightboxMetaSetup = lightbox.querySelector('.meta-setup');
  const lightboxMetaGear = lightbox.querySelector('.meta-gear');
  ...
  items.forEach(item => {
    item.addEventListener('click', () => {
      ...
      const client = item.getAttribute('data-client') || 'Industrial Partner';
      const time = item.getAttribute('data-time') || '1 Day Shoot';
      const setup = item.getAttribute('data-setup') || 'Professional Camera Rig';
      const gear = item.getAttribute('data-gear') || 'Standard Site PPE';
      const desc = item.getAttribute('data-desc') || 'Professional industrial media documentation captured under full OSHA compliance safety protocols.';
      ...
      if (lightboxMetaSetup) lightboxMetaSetup.textContent = setup;
      if (lightboxMetaGear) lightboxMetaGear.textContent = gear;
    ...
    if (formPanel) {
      formPanel.innerHTML = `
        <div style="text-align: center; padding: 4rem 2rem;">
          ...
          <h2 style="margin-bottom: 1rem;">Project Inquiry Submitted Successfully!</h2>
          <p class="lead-text" style="max-width: 600px; margin: 0 auto 2.5rem auto;">Thank you for requesting an industrial photography proposal. I will review your site locations, PPE requirements, and shoot goals, and follow up via email within 24 hours.</p>
